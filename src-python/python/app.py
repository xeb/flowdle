...
class Common:
    ...
    def showMain(self, handler, urlparam):
        ...
        orderBy = "when" 
        if subscriber and subscriber.sort:
        ...
        orderDir = "ASC"
        if subscriber and subscriber.sort_asc:
            orderDir = "ASC"
        else:
            orderDir = "DESC"
        ...
        # Send to All if Nothing has been matched
            ...
            if subscriber and subscriber.default_tag:
        ...
        handler.response.out.write(unicode(template.render('templates/app.html', values)))
        return
